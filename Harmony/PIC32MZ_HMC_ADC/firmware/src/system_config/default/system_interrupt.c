...
extern volatile bool adcFlag; //adc flag
extern uint32_t adcVal;

void __ISR(_ADC_VECTOR, ipl3AUTO) _IntHandlerDrvAdcCommonData(void)
{
    if (true == DRV_ADC_SamplesAvailable(ADCHS_AN8)) {
        adcVal = DRV_ADC_SamplesRead(ADCHS_AN8); //must read inside ISR to clear flag
        adcFlag = true;
    }
